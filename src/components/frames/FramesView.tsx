    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        ...
        >
          ...
          <p className="text-xs text-ink-400">PNG preferred · JPG · WEBP · SVG</p>
          <input
            ref={inputRef}
            type="file"
        ...
        {frames.length === 0 ? (
          <div className="rounded-lg border border-navy-700 bg-navy-900 p-8 text-center">
...
function FrameCard({
  ...
  return (
    <Panel
      className={active ? 'ring-1 ring-gold-500' : ''}
      title={active ? 'Active Frame' : 'Frame'}
    >
      <div className="mb-3 flex aspect-[4/3] items-center justify-center overflow-hidden rounded-md border border-navy-700 bg-[linear-gradient(45deg,#1e2a3a_25%,transparent_25%),linear-gradient(-45deg,#1e2a3a_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#1e2a3a_75%),linear-gradient(-45deg,transparent_75%,#1e2a3a_75%)] bg-[length:12px_12px] bg-navy-950">
        <img
          src={preview || thumb}
          alt={name}
      ...
      <p className="mb-1 truncate text-xs text-ink-400">{filename}</p>
      <p className="mb-3 text-xs text-ink-400">
        {width}×{height} · {transparent ? 'Transparency' : 'Opaque'}
      </p>
